          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: AppColors.textPrimary, letterSpacing: -0.5),
              ),
              const Text(
              ...
              Row(
                ...
                children: [
                  ...
                  TextButton(onPressed: () {}, child: const Text('View all', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold))),
                ],
              ),
              recentFilesAsync.when(
                data: (files) {
                  final filteredFiles = files.where((f) => f.namaTampilan.toLowerCase().contains(searchText.toLowerCase())).toList();
                  if (filteredFiles.isEmpty) return _buildEmptyState(searchText.isNotEmpty);
                  return ListView.separated(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    itemCount: filteredFiles.take(5).length,
                    separatorBuilder: (context, index) => const SizedBox(height: 8),
                    itemBuilder: (context, index) => _buildFileTile(filteredFiles[index]),
                  );
                },
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(file.namaTampilan, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
            Text('Uploaded · ${DateFormat.yMMMd().format(file.createdAt)}', style: const TextStyle(fontSize: 12, color: AppColors.textMuted)),
          ])),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Text(isSearch ? 'No files match your search' : 'No recent activity yet', style: const TextStyle(color: AppColors.textMuted)),
          ],
        ),
      ),
    );
